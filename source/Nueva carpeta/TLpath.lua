...
if thread:getName() == "main" then		-- we're in the main thread, so set up the handler
	...
	thread:set("require", love.filesystem.read("TSerial.lua"))	-- pass it TSerial, since threads can't read files?!?
	...
	function TLpath.calcNodes(nodes, dynamic)
		...
		if dynamic then thread:set("calcDynamicNodes", TSerial.pack(nodes)) else thread:set("calcStaticNodes", TSerial.pack(nodes)) end
	...
	function TLpath.setGfunction(f) return thread:set("runCode", "function gScore(a,b) "..f.." end") end
	...
	function TLpath.setHfunction(f) return thread:set("runCode", "function hScore(a,goal) "..f.." end") end
	...
	function TLpath.update()
		local r=thread:get("error") assert(not r, r)	-- show an error message if the thread had a problem
		local r=thread:get("status") if r then
		...
		if #TLpath.list > 0 and not TLpath.busy then
			...
			thread:set("findPath", TSerial.pack{TLpath.busy, TLpath.list[1][2]})
		...
		local r=thread:get("setPath") if r then
			r = TSerial.unpack(r)
	...
	function findPath(ent, goal, nodes)
		...
		while #openSet>0 do
			...
			local r=thread:get("cancelPath") if r then return false end		-- quit here to prevent errors if nodes are getting calced
	...
	-- watch for commands
	repeat
		local r=thread:get("runCode") if r then loadstring(r)() end
		
		local r=thread:get("calcStaticNodes") if r then
			...
			thread:set("status", "calcNodes finished")
		end
		
		local r=thread:get("calcDynamicNodes") if r then nodes=TSerial.unpack(r) thread:set("status", "calcNodes finished") end
		
		local r=thread:get("findPath") if r then
			local r2=thread:get("cancelPath")	-- clear cancel request if needed
			r=TSerial.unpack(r)
			local p = findPath(r[1],r[2],nodes)
			if p then thread:set("setPath", TSerial.pack(p)) end
		end
	until thread:get("quit")
end